...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(fields, configField{"Proxy URLs (Round Robin)", "proxy-urls", "string", getString(cfg, "proxy-urls"), nil})
	fields = append(fields, configField{"RR Include No Proxy", "proxy-round-robin-include-no-proxy", "bool", fmt.Sprintf("%v", getBool(cfg, "proxy-round-robin-include-no-proxy")), nil})
...
func fieldSection(apiPath string) string {
	...
	switch apiPath {
	case "port", "host", "debug", "proxy-url", "proxy-urls", "proxy-round-robin-include-no-proxy", "request-retry", "max-retry-interval", "force-model-prefix":
